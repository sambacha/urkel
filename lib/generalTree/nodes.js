...
class Internal extends Node {
  ...
  flags() {
    ...
    let flags = (this.prefix.size + 255) >>> 8
    /*
  ...
  getSize(hash, bits) {
    let size = 1;  //need to change this eventually
    size += 2
    size += this.flags();
    //console.log("Flags: " + this.flags())
    size += (this.prefix.size + 7) >>> 3;
    //console.log("Prefix length: " + this.prefix.data.length)
  ...
  write(data, off, hash, bits) {
    data[off] = INTERNAL;
    off += 1;
    off = writeU16(data, this.flags(), off);
    //console.log(this.prefix.data)
    //console.log("Size of prefix: " +  this.prefix.size + " "+ this.prefix.data.length)
    //Need to fix this

    off = writeUNBytes(data, this.prefix.size, off, this.flags());
  ...
  decode(data, hash, bits) {
    ...
    const type = data[off];
    off += 1;
    const flags = readU16(data, off);
    off += 2

    assert((type) === INTERNAL);
    // const size = data.slice(off, off + flags);
     const size= readUNBytes(data, off, flags);
     off += flags

     const bytes = (size + 7) >>> 3;

      const prefix = new Bits();
      //console.log("Size of prefix: " + size)
      prefix.size = size
...
class Leaf extends Node {
  ...
  getSize(hash, bits) {
    ...
    //console.log("Key size: " + len)
    let b = Bits.from(this.key)
    //console.log(b.size)

    //console.log("Key number of bytes: " + b.size)
     //should always be less than 2
    return 1 + PTR_SIZE + 1 + ((b.size + 255) >>> 8) + ((b.size + 7) >>> 3);
  }
  ...
  write(data, off, hash, bits) {
    ...
    data[off] = LEAF;
    off += 1;
    data[off] = ((b.size + 255) >>> 8)
    off += 1
    off = this.vptr.write(data, off);
    off = writeUNBytes(data, b.size, off, ((b.size + 255) >>> 8));
  ...
  decode(data, hash, bits) {
    ...
    off += 1;
    let byte_size = data[off]
    off += 1
    this.vptr = Pointer.read(data, off);
    off += PTR_SIZE;
    //let keySize = data.slice(off, off + byte_size)
    const keySize = readUNBytes(data, off, byte_size);
    off += byte_size
    this.key = data.slice(off, off + (( keySize + 7) >>> 3));
    off += (keySize + 7) >>> 3;
