...
const { AssertionError, EncodingError } = errors;
...
*/
    switch (this.type) {
      ...
      case TYPE_SHORT:
        ...
        if (this.prefix.size === 0) return false;
        /*
        if (this.prefix.size > bits)
          return false;
*/
        if (this.left.length !== hash.size) return false;

        if (this.right.length !== hash.size) return false;
      ...
      case TYPE_COLLISION:
        ...
        if (this.key.length !== (bits >>> 3))
          return false;
*/
        if (this.hash.length !== hash.size) return false;
      ...
      case TYPE_EXISTS:
        ...
        if (this.value.length > 0xffff) return false;
  ...
  verify(root, key, hash, bits) {
    ...
    if (!this.isSane(hash, bits)) return [PROOF_UNKNOWN_ERROR, null];
    ...
    switch (this.type) {
      case TYPE_DEADEND:
        ...
        break;
      case TYPE_SHORT:
        if (this.prefix.has(key, this.depth)) return [PROOF_SAME_PATH, null];
        leaf = hashInternal(hash, this.prefix, this.left, this.right);
        break;
      case TYPE_COLLISION:
        if (this.key.equals(key)) return [PROOF_SAME_KEY, null];
    ...
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      ...
      if (depth < prefix.size + 1) return [PROOF_NEG_DEPTH, null];

      depth -= 1;

      if (hasBit(key, depth)) next = hashInternal(hash, prefix, node, next);
      else next = hashInternal(hash, prefix, next, node);

      depth -= prefix.size;

      if (!prefix.has(key, depth)) return [PROOF_PATH_MISMATCH, null];
    }

    if (depth !== 0) return [PROOF_TOO_DEEP, null];

    if (!next.equals(root)) return [PROOF_HASH_MISMATCH, null];
  ...
  getSize(hash, bits) {
    ...
    for (const { prefix, node } of this.nodes) {
      if (prefix.size !== 0) size += prefix.getSize();
  ...
  writeBW(bw, hash, bits) {
    ...
    if (bw.data) bw.offset = this.write(bw.data, bw.offset, hash, bits);
    else bw.writeBytes(this.encode(hash, bits));
    return bw;
  }
  ...
  read(data, off, hash, bits) {
    ...
    if (this.depth > bits) throw new EncodingError(pos, "Invalid depth");
    ...
    if (count > bits) throw new EncodingError(pos, "Proof too large");
...
*/
  ...
  static type(value) {
    ...
    if (value >= typesByVal.length) value = TYPE_UNKNOWN;
  ...
  static code(value) {
    ...
    if (value >= codesByVal.length) value = PROOF_UNKNOWN_ERROR;
...
function checkWrite(ok, offset, start) {
  if (!ok) {
    throw new EncodingError(offset, "Out of bounds write", start || checkWrite);
  }
}

function checkRead(ok, offset, start) {
  if (!ok) {
    throw new EncodingError(offset, "Out of bounds read", start || checkRead);
  }
}
