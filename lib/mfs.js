...
const info = os.userInfo ? os.userInfo() : { uid: 0, gid: 0 };
...
class MFS {
  ...
  _find(path) {
    ...
    if (names.length === 0) return [this.root, null, ""];
    ...
    for (const name of names) {
      ...
      if (!node) throw makeError("stat", "ENOENT", path);

      if (!node.isDirectory()) throw makeError("stat", "ENOENT", path);
    }
  ...
  openSync(path, flags, mode) {
    ...
    switch (flags) {
      ...
      case "ax+":
      case "wx+":
        if (file) throw makeError("open", "EEXIST", path);
        break;
      case "r":
      case "r+":
      case "rs+":
        if (!file) throw makeError("open", "ENOENT", path);
        break;
    }
    ...
    if (file.isDirectory()) throw makeError("open", "EISDIR", path);
  ...
  closeSync(fd) {
    ...
    if (!this.fds.has(fd)) throw makeError("close", "EBADF");
  ...
  readdirSync(path) {
    ...
    if (!file) throw makeError("readdir", "ENOENT", path);

    if (!file.isDirectory()) throw makeError("readdir", "ENOTDIR", path);
  ...
  mkdirSync(path, mode) {
    if (mode == null) mode = 0o777;

    const [file, parent, name] = this._find(path);

    if (file) throw makeError("mkdir", "EEXIST", path);

    if (!parent) throw makeError("mkdir", "ENOENT", path);
  ...
  mkdirpSync(path, mode) {
    ...
    for (const name of names) {
      ...
      if (!node.isDirectory()) throw makeError("mkdir", "ENOENT", path);
    }

    const file = node.map.get(name);

    if (file) {
      if (!file.isDirectory()) throw makeError("mkdir", "ENOTDIR", path);
      return;
    }
  ...
  rmdirSync(path) {
    ...
    */
    if (!file) throw makeError("rmdir", "ENOENT", path);

    if (!file.isDirectory()) throw makeError("rmdir", "ENOTDIR", path);
    //

    if (file.map.size > 0) throw makeError("rmdir", "ENOTEMPTY", path);
  ...
  renameSync(path, to) {
    ...
    if (!file1 || !parent1) throw makeError("rename", "ENOENT", path);

    const [file2, parent2, name2] = this._find(to);

    if (!parent2) throw makeError("rename", "ENOENT", path);

    if (file2) throw makeError("rename", "EEXIST", path);
  ...
  statSync(path) {
    ...
    if (!file) throw makeError("stat", "ENOENT", path);
  ...
  chmodSync(path, mode) {
    ...
    if (!file) throw makeError("chmod", "ENOENT", path);
  ...
  chownSync(path, uid, gid) {
    ...
    //
    if (!file) throw makeError("chown", "ENOENT", path);
  ...
  unlinkSync(path) {
    ...
    if (!file || !parent) throw makeError("unlink", "ENOENT", path);

    if (file.isDirectory()) throw makeError("unlink", "EISDIR", path);
  ...
  utimesSync(path, atime, mtime) {
    ...
    if (!file) throw makeError("utimes", "ENOENT", path);
  ...
  truncateSync(path, size) {
    ...
    if (!file) throw makeError("truncate", "ENOENT", path);
  ...
  futimesSync(fd, atime, mtime) {
    ...
    if (!item) throw makeError("futimes", "EBADF");
  ...
  fstatSync(fd) {
    ...
    if (!item) throw makeError("fstat", "EBADF");
  ...
  ftruncateSync(fd, size) {
    ...
    if (!item) throw makeError("ftruncate", "EBADF");

    if (!item.writable) throw makeError("ftruncate", "EBADF");
  ...
  fsyncSync(fd) {
    ...
    if (!item) throw makeError("fsync", "EBADF");

    if (!item.writable) throw makeError("fsync", "EBADF");
  }
  ...
  readSync(fd, data, offset, length, position) {
    ...
    if (!item) throw makeError("read", "EBADF");

    if (!item.readable) throw makeError("read", "EBADF");
  ...
  writeSync(fd, data, offset, length, position) {
    ...
    if (!item) throw makeError("write", "EBADF");

    if (!item.writable) throw makeError("write", "EBADF");
...
class RegularFile extends File {
  ...
  expand(size) {
    if (this.data.length === 0) this.data = Buffer.allocUnsafe(8192);
  ...
  write(data, offset, length, position) {
    ...
    const pos = typeof position === "number" ? position : this.size;
