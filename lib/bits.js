...
const { EMPTY, hasBit, setBit } = common;
...
class Bits {
  ...
  _count(index, key, depth) {
    ...
    for (let i = 0; i < len; i++) {
      if (this.get(index) !== hasBit(key, depth)) break;
  ...
  slice(start, end) {
    ...
    if (size === 0) return Bits.EMPTY;

    const bits = this.constructor.alloc(size);

    for (let i = start, j = 0; i < end; i++, j++) bits.set(j, this.get(i));

    return bits;
  }

  split(index) {
    return [this.slice(0, index), this.slice(index + 1, this.size)];
  }
  ...
  toString() {
    ...
    for (let i = 0; i < this.size; i++) str += this.get(i);
