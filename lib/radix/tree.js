...
const { hasBit, hashValue, randomPath } = common;

const { MissingNodeError, AssertionError } = errors;

const { types, Node, NIL, Internal, Leaf } = nodes;

const { NULL, INTERNAL, LEAF, HASH } = types;

const { FileStore, MemoryStore } = store;
...
class Tree {
  ...
  async open(root) {
    ...
    if (root) await this.inject(root);
  }
    ...
    for (;;) {
      switch (node.type()) {
        ...
        case INTERNAL: {
          ...
          if (!node.prefix.has(key, depth)) return null;
        ...
        case LEAF: {
          // Prefix collision.
          if (!key.equals(node.key)) return null;
    ...
    switch (node.type()) {
      ...
      case LEAF: {
        ...
        if (key.equals(node.key)) {
          ...
          if (value.equals(node.data)) return null;
  ...
  async _remove(node, side, key, depth) {
    switch (node.type()) {
      ...
      case INTERNAL: {
        ...
        if (!node.prefix.has(key, depth)) return [null, 0];
      ...
      case LEAF: {
        // Not our key.
        if (!key.equals(node.key)) return [null, 0];
...
class Snapshot {
  ...
  async getRoot() {
    if (!this.root) this.root = await this.tree.getHistory(this.hash);
    return this.root;
  }
  ...
  async get(key) {
    ...
    if (!this.root) this.root = await this.getRoot();
    ...
    outer: for (;;) {
      ...
      switch (node.type()) {
        ...
        case INTERNAL: {
          ...
          if (parent.child) this.push(node.right, depth + node.prefix.size + 1);
          else this.push(node.left, depth + node.prefix.size + 1);
