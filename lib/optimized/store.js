...
const { AssertionError, MissingNodeError } = errors;

const { NIL, Internal, Leaf } = nodes;
...
const MAX_FILE_SIZE = 0x7ffff000; // File max = 2 GB    2147479552 69657815
...
class Store {
  ...
  async stat() {
    ...
    for (const { size } of files) total += size;
  ...
  async ensure() {
    if (this.index !== 0) throw new Error("Store already opened.");
  ...
  async close() {
    if (this.index === 0) throw new Error("Store already closed.");
    ...
    for (const file of files.values()) await file.close();

    return this.lockFile.close();
  }

  async destroy() {
    if (this.index !== 0) throw new Error("Store is opened.");
    ...
    for (const { path } of files) await this.fs.unlink(path);
  ...
  async openFile(index, flags) {
    ...
    if (this.index === 0) throw new Error("Store is closed.");
  ...
  async _reallyOpen(index, flags) {
    ...
    if (this.files.size >= MAX_OPEN_FILES) await this.evict();
  ...
  openFileSync(index, flags) {
    ...
    if (this.files.size >= MAX_OPEN_FILES) this.evictSync();
  ...
  async closeFile(index) {
    ...
    if (file.reads > 0) return undefined;
  ...
  writeNode(node) {
    ...
    const size = node.isLeaf() ? this.leafSize : this.nodeSize;
  ...
  async flush() {
    for (const chunk of this.buffer.flush()) await this.write(chunk);
  ...
  async findMeta(file, slab) {
    ...
    while (off >= META_SIZE) {
      ...
      while (size >= META_SIZE) {
        ...
        if (readU32(data, size) !== META_MAGIC) continue;
  ...
  async recoverState(index) {
    ...
    if (this.index !== 0) throw new Error("Store is open.");

    if (index === 0) return [new Meta(), new Meta()];
  ...
  async getRoot() {
    ...
    if (this.state.rootNode) return this.state.rootNode;
  ...
  async readRoot(index, pos, leaf) {
    ...
    if (!this.rootCache.has(key)) this.rootCache.set(key, root);
  ...
  async getHistory(rootHash) {
    ...
    if (rootHash.equals(this.hash.zero)) return NIL;

    const root = this.state.rootNode;
    const hash = root.hash(this.hash);

    if (rootHash.equals(hash)) return root;
  ...
  async _readHistory(rootHash) {
    ...
    for (;;) {
      ...
      if (hash.equals(rootHash)) return node;
...
class Meta {
  ...
  read(data, off, hash, key) {
    ...
    if (magic !== META_MAGIC) throw new Error("Invalid magic number.");
    ...
    if (!chk.equals(expect)) throw new Error("Invalid metadata checksum.");
...
class WriteBuffer {
  ...
  flush() {
    ...
    if (this.written > this.start) chunks.push(this.render());
