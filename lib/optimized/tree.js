...
class Tree {
    ...
    for (;;) {
      switch (node.type()) {
        ...
        case INTERNAL: {
          ...
          // Internal node.
          if (hasBit(key, depth)) node = node.right;
          else node = node.left;
  ...
  async _insert(root, key, value) {
      ...
      switch (node.type()) {
        ...
        case LEAF: {
          ...
          if (key.equals(node.key)) {
            ...
            if (leaf.equals(node.data)) return root;
    ...
    while (nodes.length > 0) {
      ...
      if (hasBit(key, depth)) root = new Internal(node, root);
      else root = new Internal(root, node);
    }
  ...
  async _remove(root, key) {
    ...
    outer: for (;;) {
      switch (node.type()) {
        ...
        case LEAF: {
          // Not our key.
          if (!key.equals(node.key)) return root;
  ...
  _write(node) {
    ...
    switch (node.type()) {
      ...
      case INTERNAL: {
        ...
        //console.log("Node index 1 ", node.index)
        assert(node.index !== 0);
        //console.log("Node index: ", node.index)
