...
class Proof {
  ...
  isSane(hash, bits) {
    ...
    if (this.nodes.length > bits) return false;

    switch (this.type) {
      case TYPE_DEADEND:
        if (this.key || this.hash || this.value) return false;
        break;
      case TYPE_COLLISION:
        if (
          !this.key ||
          !this.hash ||
          this.value ||
          this.key.length !== bits >>> 3 ||
          this.hash.length !== hash.size
        )
          return false;
        break;
      case TYPE_EXISTS:
  ...
  verify(root, key, hash, bits) {
    ...
    for (let i = this.nodes.length - 1; i >= 0; i--) {
      ...
      if (hasBit(key, i)) next = hashInternal(hash, node, next);
      else next = hashInternal(hash, next, node);
    }
  ...
  getSize(hash, bits) {
    ...
    for (const node of this.nodes) {
      if (!node.equals(hash.zero)) size += node.length;
    }
  ...
  fromJSON(json, hash, bits) {
    ...
    if (json.nodes.length > bits) throw new EncodingError(0, "Proof too large");

    this.type = type;

    for (const item of json.nodes) this.nodes.push(parseHex(item, hash.size));
