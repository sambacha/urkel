...
async function stress(prefix) {
...

  {
    ...
    try {
      ...
      if (e.code !== "ERR_MISSING_NODE") throw e;
      missing = true;
    }
  ...
  for (let i = 0; i < BLOCKS; i++) {
...

    {
      ...
      for (const [k, v] of pairs) await batch.insert(k, v);
    ...
    if (i % 100 === 0) console.log("Keys: %d", i * PER_BLOCK);
  }
...
async function doProof(tree, i, key, expect) {
  ...
  if (proof.nodes.length > 0) {
    if (Buffer.isBuffer(proof.nodes[0])) {
      for (const node of proof.nodes) size += 2 + node.length;
    } else {
  ...
  console.log(
    "Proof %d compressed size: %d",
    i,
    proof.getSize(tree.hash, tree.bits)
  );
}
...
async function bench(prefix) {
  ...
  {
    const now = util.now();
    for (const [key] of items) assert(await batch.get(key));
  ...
  {
    const now = util.now();
    for (const [key] of items) assert(await tree.get(key));
...
(async () => {
  const arg = argv.length >= 3 ? argv[2] : "";
